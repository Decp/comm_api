...
int getDevPath(const char *buf,char *path,int size)
{
	...
	for(i = 1;i <= 8;i++)
	{
		...
		if(strstr(buf,tmp))
		{
			strcpy(path,"/dev");
			strcat(path,tmp);
			printf("\033[33m getDevPath:%s \033[0m \n",path);
			return 0;
		}else
			continue;
	}
	
	sprintf(tmp,"/wlan%d/queues/tx-3n",0);
	if(strstr(buf,tmp))
	{
		//strcpy(path,"/dev");
		//strcat(path,tmp);
		printf("\033[33m getDev type:%s \033[0m \n",tmp);
...
void check()
{
	...
	DIR *dir = opendir("/mnt/card/");
	while((file=readdir(dir))!=NULL)
	{
		sprintf(tmp,"/mnt/card/%s",file->d_name);
		ret = rmdir(tmp);
		if(ret == 0)
	...
	{ 
		...
		if(ret != -1)
		{
			...
			printf("label:%s\n",label);
			sprintf(dstPath,"/mnt/card/%s",label);
...
int main(int argc,char *argv[])
{
	...
	//		printf("buf content is %s\n",buf);
			if(getDevPath(buf,devPath,30)==0)
			{
				...
				sprintf(dstPath,"/mnt/card/%s",vol);
				make_dir(devPath,dstPath);
				sprintf(comm,"mount -t auto -o utf8,rw %s %s",devPath,dstPath);
				system(comm);
				event.event = ADD_EVENT;
	...
	//		printf("ACTION=remove\n");
			...
			{
			// sprintf(comm,"umount %s",devPath);
				if(strcmp(devPath,"/dev/sda1") ==0)
				{
					sprintf(comm,"umount %s","/mnt/card/usb1");
					system(comm);
					rm_dir("/mnt/card/usb1");
